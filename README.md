...
5. 在「擴充功能」欄找到並展開【酒館桌寵】，按「開啟桌寵設定」。
6. 設定會在獨立 HTML 頁面開啟；用醒目的匯入按鈕選擇 `.jrpack.zip`，再按「綁定目前角色」。

酒館會直接下載這個 repo；根目錄已包含 `manifest.json` 和預先建置的 `dist/`，使用者不需要安裝 Node.js 或自行編譯。

擴充程式欄只保留設定、桌寵開關、來信日記與番外留言板的快捷入口，不會被整份設定卡住。頁面載入時入口會顯示進度；如果失敗，也會在原地顯示可讀錯誤。

## 已包含

- 安全 data-only 角色包匯入；拒絕 JS、HTML、SVG、假 PNG、危險 ZIP 路徑。
- 每個角色獨立綁定或解除綁定角色包；解除綁定不會刪除角色包與歷史。
- 【酒館桌寵】擴充項目只提供精簡入口；完整設定在獨立 HTML 頁完成。
- 沿用目前酒館 API，或選擇酒館已存在的 Connection Profile；Loader 不保存 API Key。
- 桌機／手機大小、透明度、拖曳位置、動畫速度與移動速度。
- 點桌寵可分別開啟「角色來信日記」與「對話番外留言板」；只有在設定頁明示按生成才呼叫模型。
- 日常陪伴 Prompt 可見可改；按「讓桌寵說一句」才會帶角色卡描述／性格／情境與指定最近樓數生成泡泡，自動生成預設關閉。
- 已綁定角色可在日常、來信、番外各自勾選目前酒館載入的「常駐且未停用」世界書條目；Loader 只讀取並加入 Prompt，不修改世界書。
- 書信與番外結果保存在各自的純閱讀 HTML 頁；來信採月份＋日期軸信紙，番外採粉／紫／薄荷／霧藍輪替留言卡。兩者依角色＋聊天＋功能隔離，重開仍在，並可各自下載成累積 TXT。
