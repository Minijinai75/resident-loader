...
- 2026-08-14 23:45：來信頁參考 Mini 提供的手機日記圖，採「月份標題＋左側日期欄＋右側信紙」的窄版垂直閱讀流；日期欄使用內嵌 SVG 信封，不新增外部圖片或網路字型。番外頁採固定順序輪替的奶油粉、淡紫、薄荷與霧藍留言卡，每則保留日期、正文、複製與刪除。兩頁不增加 Prompt／連線設定，手機維持 8px 安全邊與至少 48px 操作高度。
...
- 2026-08-14 22:48：角色已綁桌寵時，日常／來信／番外各自列出 ST 1.18.0 `getSortedEntries()` 回傳中 `constant === true && disable !== true` 的世界書條目供 USER 勾選。只把被勾選的正文加入該功能 Prompt；不修改條目、不切換世界書、不替 USER 自動勾選。選擇以 `world::uid` 保存於該角色設定。
